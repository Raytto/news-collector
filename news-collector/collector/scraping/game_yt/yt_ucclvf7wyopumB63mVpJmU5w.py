...
CHANNEL_ID = "UCCLVf7wyOpUmB63mVpJmU5w"
SOURCE = "youtube_gaming_mobile"
CATEGORY = "game_yt"
TIMEOUT = 10
...
UA = (
    ...
    "(KHTML, like Gecko) Chrome/120.0 Safari/120.0"
)
...
def _pick_html_screenshots(html_txt: str) -> List[str]:
    urls: List[str] = []
    pat = re.compile(
...
def _fetch_itunes_meta(store_url: str) -> Optional[StoreMeta]:
    if store_url in _STORE_CACHE:
        cached = _STORE_CACHE[store_url]
        return StoreMeta(cached.get("title", ""), cached.get("description", ""), cached.get("screenshots", []))
...
def _fetch_gp_meta(store_url: str) -> Optional[StoreMeta]:
    ...
    _STORE_CACHE[key] = {"title": title, "description": description, "screenshots": screenshots}
    if not title:
        return None
    return StoreMeta(title=title, description=description, screenshots=screenshots)
...
def _fetch_store_meta(store_url: str, platform: str) -> Optional[StoreMeta]:
    if platform == "gp":
        return _fetch_gp_meta(store_url)
    return _fetch_itunes_meta(store_url)


def _save_screenshot(track_id: str, video_id: str, idx: int, shots: List[str], max_vertical: int = 3) -> str:
...
def collect_latest() -> List[Dict[str, str]]:
    ...
    for entry in getattr(feed, "entries", []):
        ...
        for idx, (store_url, platform) in enumerate(store_links[:MAX_LINKS_PER_VIDEO], start=1):
            ...
            track_match = re.search(r"id=([\w\.]+)", store_url) or re.search(r"id(\d+)", store_url)
            track_id = track_match.group(1) if track_match else vid
            img_path = ""
            if meta.screenshots:
