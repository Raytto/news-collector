...
def _parse_store_links(desc: str) -> List[Tuple[str, str]]:
    ...
    itunes_pat = re.compile(r"https?://(?:apps|itunes)\.apple\.com/[^\s)<>]+", re.IGNORECASE)
    gp_pat = re.compile(r"https?://play\.google\.com/store/apps/details\?id=[\w\.\-]+", re.IGNORECASE)
    for m in itunes_pat.finditer(desc):
        url = m.group(0).strip().rstrip(")")
        links.append((url, "itunes"))
    for m in gp_pat.finditer(desc):
        url = m.group(0).strip().rstrip(")")
        links.append((url, "gp"))
    # Handle YouTube redirect wrapped GP links
    redirect_pat = re.compile(r"https?://www\.youtube\.com/redirect[^\s)]+", re.IGNORECASE)
    for m in redirect_pat.finditer(desc):
        url = m.group(0)
        qs = parse_qs(urlparse(url).query)
        targets = qs.get("q") or qs.get("url") or []
        if targets:
            target = targets[0]
...
def _normalize_shot_url(url: str) -> str:
    ...
    if "{w}" in u and "{h}" in u:
        u = re.sub(r"\{w\}x\{h\}(?:\{c\})?(?:\.\{f\})?", "900x900bb.jpg", u)
    u = re.sub(r"=w\d{2,4}-h\d{2,4}.*", "", u)
    return u
...
def _pick_html_screenshots(html: str) -> List[str]:
    urls: List[str] = []
    pat = re.compile(r"https://is\d-ssl\.mzstatic\.com/image/[^\s\"']+|https://play-lh\.googleusercontent\.com/[^\s\"']+")
    ...
    for m in pat.finditer(html):
        ...
        base = msize.group(1) if msize else re.sub(r"=w\d{2,4}-h\d{2,4}.*", "", url)
        w, h = _extract_img_size_from_url(raw_url)
        if max(w, h) < 500:
            continue
        prev = best.get(base)
        area = w * h
...
def _fetch_itunes_meta(store_url: str) -> Optional[StoreMeta]:
    ...
    if not title:
        return None
    return StoreMeta(title=title, description=description, screenshots=screenshots, platform="itunes")
...
def _fetch_gp_meta(store_url: str) -> Optional[StoreMeta]:
    ...
    if not title:
        return None
    return StoreMeta(title=title, description=description, screenshots=screenshots, platform="gp")


def _fetch_store_meta(store_url: str, platform: str) -> Optional[StoreMeta]:
    key = f"{platform}:{store_url}"
    if key in _STORE_CACHE:
        data = _STORE_CACHE[key]
        return StoreMeta(data.get("title", ""), data.get("description", ""), data.get("screenshots", []), platform)
...
def _save_screenshot(track_id: str, video_id: str, idx: int, shots: List[str], max_vertical: int = 4) -> str:
    TEMP_DIR.mkdir(parents=True, exist_ok=True)
    images: List[Image.Image] = []
    for url in shots[:max_vertical]:
    ...
    if first.width >= first.height:
        ...
        return str(out_path)
    stitched = _concat_vertical(images[:max_vertical])
    if stitched:
...
def _build_video_link(base_link: str, timestamp: Optional[int], index: int) -> str:
    if timestamp is not None:
        return f"{base_link}&t={timestamp}s" if "?" in base_link else f"{base_link}?t={timestamp}s"
    sep = "&" if "?" in base_link else "?"
    return f"{base_link}{sep}index={index}"
...
def collect_latest() -> List[Dict[str, str]]:
    ...
    for entry in getattr(feed, "entries", []):
        ...
        for idx, (store_url, platform) in enumerate(store_links[:MAX_LINKS_PER_VIDEO], start=1):
            ...
            track_id = re.search(r"id=([\w\.]+)", store_url)
            track = track_id.group(1) if track_id else vid
            img_path = ""
            if meta.screenshots:
                max_vert = 3 if platform == "gp" else 4
                img_path = _save_screenshot(track, vid, idx, meta.screenshots, max_vertical=max_vert)
