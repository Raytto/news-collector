...
RSS_URL = "https://indienova.com/feed/"
SOURCE = "indienova"
CATEGORY = "game"
...
# Cache the latest feed entry details to avoid refetching per-URL during the same run
...
def _extract_content(entry: Any) -> str:
    ...
    # Fallback to summary_detail / summary when present
    sd = entry.get("summary_detail") or {}
    if isinstance(sd, dict) and sd.get("type", "").startswith("text/"):
        return _clean_text(sd.get("value") or "")
...
def collect_entries(feed: Any, limit: int = 10) -> List[Dict[str, str]]:
    ...
    for e in getattr(feed, "entries", []):
        ...
        # Pre-extract and cache cleaned detail from the feed
...
def process_entries(feed: Any) -> List[Dict[str, str]]:
    # Compatibility alias for manager; keep default limit
    return collect_entries(feed, limit=50)


def fetch_article_detail(url: str) -> str:
    # Prefer cached detail from the most recent feed fetch
    cached = _DETAIL_CACHE.get(url)
    if cached:
        return cached

    # As a fallback, re-fetch the feed and search for the entry
...
def main(limit: int = 10) -> None:
    ...
    for it in items:
        print(f"{it['source']} - {it['published']} - {it['title']} - {it['url']}")
