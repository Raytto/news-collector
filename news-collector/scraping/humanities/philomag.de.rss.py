...
RSS_URL = "https://www.philomag.de/rss.xml"
SOURCE = "philomag.de"
CATEGORY = "humanities"
...
def _extract_content(entry: Any) -> str:
    ...
    if isinstance(sd, dict) and sd.get("value"):
        # summary_detail may already be text/plain
        val = sd.get("value") or ""
...
def _pick_main(soup: BeautifulSoup):
    selectors = [
        # Guessed/typical article containers
...
def fetch_article_detail(url: str) -> str:
    ...
    # Fallback 1: re-fetch feed and search by link
    try:
    ...
    # Fallback 2: final attempt by fetching the article page
