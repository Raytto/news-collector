...
FEED_URL = "https://a16z.substack.com/feed"
UA = (
...
def fetch_feed(url: str = FEED_URL) -> feedparser.FeedParserDict:
    """Fetch the RSS feed for a16z on Substack."""
...
def _normalize_url(link: str) -> str:
    ...
    if not text.startswith("/"):
        text = "/" + text
    return f"https://a16z.substack.com{text}"
...
def _strip_noise(soup: BeautifulSoup) -> None:
    ...
    for noisy in soup.select(
        "[aria-label*='share' i], [class*='share' i], [class*='subscribe' i]"
    ):
        noisy.decompose()
