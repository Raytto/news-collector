...
# Force email delivery to use plain text only (no HTML part).

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
PYTHON="${PYTHON:-python}"
ENV_NAME="news-collector"

# Timestamped logging (Asia/Shanghai) to log/<ts>-auto-once-txt-log.txt
TS="$(TZ='Asia/Shanghai' date '+%Y%m%d-%H%M%S')"
LOG_DIR="$ROOT_DIR/log"
LOG_FILE="$LOG_DIR/${TS}-auto-once-txt-log.txt"
mkdir -p "$LOG_DIR"
exec > >(tee -a "$LOG_FILE") 2>&1
echo "[INFO] Log file: $LOG_FILE"

# Enforce plain-text email delivery for this run
export MAIL_PLAIN_ONLY=1
echo "[INFO] MAIL_PLAIN_ONLY=$MAIL_PLAIN_ONLY"
...
run_once() {
  ...
  echo "[INFO] Running all pipelines sequentially (plain-text delivery)..." >&2
