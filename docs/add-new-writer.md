...
本文档约定在 `news-collector/writer/` 目录内新增“Writer”类脚本时需遵守的规范，确保输出样式、依赖和 CLI 行为与现有 `email_writer.py`、`feishu_writer.py` 一致。
...
## 2. 数据来源与依赖
- 默认数据源为 SQLite `data/info.db`，Writer 应通过 SQL 一次性拉取所需字段（`info`、`info_ai_review`、`info_ai_scores`、`ai_metrics`）。
- 维度定义来源于 `ai_metrics` 表；新增或调整维度需先更新 `ai_metrics` 管理逻辑（见 AI 评估规范）并同步默认权重。
...
## 4. 评分与排序
- Writer 必须在展示阶段重新计算加权总分，而非直接依赖 `info_ai_review.final_score`。
- 维度与中文标签应实时读取 `ai_metrics(active=1 ORDER BY sort_order)`；仅对权重大于 0 的维度参与加权。
...
## 7. 与现有 Writer 对齐的要点
- `email_writer.py`：生成 HTML 卡片视图；按分类分组，展示星级、维度分、AI 评语与概要。
- `feishu_writer.py`：输出 Lark Markdown，强调序号、评分与超链接；与 `deliver/feishu_deliver.py` 配合发送。
- 新 Writer 若与上述脚本共享逻辑（例如权重解析、分数聚合），优先复用或提取公共函数，避免复制粘贴后出现漂移。
